...
var startItems = ``;
for(var z = 0; z<4; z++){
    var randomFirst = Math.floor(Math.random()*5) + 1;
    var itemRandom = Math.floor(Math.random() * itemList[randomFirst].length)
    startItems += `<div class="item-common rare-level-${randomFirst}"><div class="item class_red_item" style="background-image:url('${itemList[randomFirst][itemRandom].img}');"></div></div>`
}
$('.raffle-roller-container').html(startItems)